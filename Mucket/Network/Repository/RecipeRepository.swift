...
protocol RecipeRepositoryType {
    ...
    func search(byIngredient: String) async throws -> [RecipeEntity]
...
final class RecipeRepository: RecipeRepositoryType {
    ...
    func search(byIngredient ingredient: String) async throws -> [RecipeEntity] {
        do {
            let result: RecipeDTO = try await networkManager.fetchData(.searchRecipe(ingredient))
            return result.recipeInfo.row.map { $0.toEntity() }
        } catch {
            throw error
        }
    ...
    func fetchTheme(type: String) async throws -> [RecipeEntity] {
        do {
            let result: RecipeDTO = try await networkManager.fetchData(.fetchThemedRecipe(type))
            return result.recipeInfo.row.map { $0.toEntity() }
        } catch {
            throw error
        }
