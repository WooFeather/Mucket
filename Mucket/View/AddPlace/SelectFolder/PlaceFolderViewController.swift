//
//  PlaceFolderViewController.swift
//  Mucket
//
...
final class PlaceFolderViewController: BaseViewController {
    private let selectFolderView = SelectFolderView()
    var disposeBag = DisposeBag()
    var onFolderSelected: ((PlaceFolderEntity) -> Void)?
    
    init(reactor: PlaceFolderReactor) {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        Observable.just(())
            .map { PlaceFolderReactor.Action.viewWillAppear }
    ...
    override func configureData() {
        selectFolderView.folderTableView.register(UITableViewCell.self, forCellReuseIdentifier: "placeFolderTableViewCell")
    }
}

extension PlaceFolderViewController: View {
    func bind(reactor: PlaceFolderReactor) {
        bindAction(reactor)
        bindState(reactor)
    }
    
    private func bindAction(_ reactor: PlaceFolderReactor) {
        selectFolderView.folderTableView.rx.modelSelected(PlaceFolderEntity.self)
            ...
            .bind(with: self) { owner, indexPath in
                ...
                if folder.name == "기본 폴더" {
                    ...
                    owner.showAlert(title: "폴더 삭제", message: "해당 폴더에 속한 요리는 '기본 폴더'로 이동하게 됩니다. 폴더를 삭제하시겠습니까?", button: "삭제", style: .destructive, isCancelButton: true) {
                        ...
                        NotificationCenter.default.post(name: NSNotification.Name("PlaceFolderListUpdated"), object: nil)
                    }
                }
            }
            .disposed(by: disposeBag)
    ...
    private func bindState(_ reactor: PlaceFolderReactor) {
        reactor.state
            .map { $0.folderList }
